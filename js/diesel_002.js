	...
	open: function() {
		...
		$(window).smartresize(function(){
			...
			if($(window).height() < options.height){
				$('.ui-dialog').css('height',$(window).height() - 20); 
				$('#dialog-container').css('height',$(window).height() - 60); 
			}
			
			 var kindleBrowserSilk = /\bSilk\b/,kindleModel = /\bKFTT\b/,isKindle = false;
             // This code is commented because of jira - 2329 
			 /*if ((kindleBrowserSilk.test(navigator.userAgent) && kindleModel.test(navigator.userAgent))) {
                if($('.ui-dialog input').length){
		...
		$('.ui-widget-overlay').off('touchstart').on('touchmove',function(e){
			e.preventDefault();
		...
		$('.ui-dialog').off('touchstart').on('touchstart',function(e){
			$(document).off('touchmove');
		});
 ...
 * jQuery select arrow 
 */
$(document).ready(function(){customizeSelectNow();});function customizeSelectNow(){if(!scriptIsmobile()){if($.browser.msie){if(parseInt($.browser.version,10)!=8){var selectEle=$('body').find('select');for(var i=0;i<selectEle.length;i++){var ele=$(selectEle[i]);customizeSelect(ele);}}}else{var selectEle=$('body').find('select');for(var i=0;i<selectEle.length;i++){var ele=$(selectEle[i]);customizeSelect(ele);}}}}
function customizeSelect(ele){var parent=ele.parent();parent.find('.select-img').remove();parent.append('<span class="select-img"></span>');var span=ele.parent().find('.select-img');parent.css('position','relative');span.offset({position:'absolute',top:ele.offset().top,left:ele.offset().left});ele.css({'-webkit-appearance':'none'})
var width=(((ele.outerWidth()-2)*100)/parent.outerWidth())+'%';span.css({'width':width,'height':ele.outerHeight()});$(window).resize(function(){var width=(((ele.outerWidth()-2)*100)/parent.outerWidth())+'%';span.css({'width':width,'height':ele.outerHeight()});});var style="";span.click(function(e){var $target=$(e.target).parent().find('select');var $clone=$target.clone(),select_size;var padding="0px 0px 0px "+(parseInt(ele.css('padding-left').substr(0,(ele.css('padding-left').length-2)))+1)+"px";$clone.val($target.val()).css({position:'absolute','z-index':999,left:$target.offset().left,top:$target.offset().top+$target.outerHeight(),width:$target.outerWidth(),'overflow-y':'auto','padding':padding,'color':ele.css('color')});select_size=$clone.find('option').length;if(select_size>=2&&select_size<=4){select_size=select_size*18;$clone.css('height','auto');}
$clone.attr('size',($clone.find('option').length>10?10:$clone.find('option').length)).change(function(){$target.val($clone.val());$target.change();}).on('click blur',function(){$(style).remove();$(this).remove();});$($clone.find('option')).hover(function(){$(this).css('background','#ccc');},function(){$(this).css('background','none');});$('body').append($clone);setTimeout(function(){$clone.focus();},100);});}
/*
	...
	validate: function( options ) {
		...
		if ( validator.settings.onsubmit ) {
			...
			this.submit( function( event ) {
				...
				globalValidationResult =  validator.form();
...
;( function( $ ) 
{
    ...
    function Scrollbar( root, options )
    {
        var oSelf       = this
        ,   oWrapper    = root
      //,   oViewport   = { obj: $( '.viewport:first', root ) }
     // ,   oContent    = { obj: $( '.overview', root ) }
     // ,   oScrollbar  = { obj: $( '.scrollbar', root ) }
        ,   oViewport   = { obj: $( '.viewport:first', root ) }
        ,   oContent    = { obj: $( '.overview:first', root ) }
        ,   oScrollbar  = { obj: $( '.scrollbar:first', root ) }
        ...
        function setEvents()
        {
            ...
            else
            {
               ...
               $(oViewport.obj[0]).on('touchstart',function( event )
                {   
        ...
        function start( event )
        {
            ...
            {
            	 $(oViewport.obj[0]).on('touchmove', function( event )
                {
                	 var touch = event.originalEvent.touches[0] || event.originalEvent.changedTouches[0];
                    event.preventDefault();
                    drag( touch );
 ...
 * */
(function($) {
	//this : div wrapper for ul and input[type='text']
	//arrows : selector box arrow
	$.fn.customSelectBox = function() {
        ...
        if (kindleBrowserSilk.test(navigator.userAgent) && kindleModel.test(navigator.userAgent)) {
		 ...
		 $(this).each(function(){
			var	list = $(this).find("div.customSelectBoxScroll"),
				item = $(this).find("li"),
				textField =  $(this).find(".selectedVal"),
				arrow = $(this).find('.arrows'),
				parentAttr = $(this).parents('.attribute'),
				productVariationDiv = $(this).parents('.product-variations');
				//setting height of viewport
				var itemNo =  item.length;
				if(itemNo >= 5){
					list.find('.viewport').css('height','125px');
				}
				else {
					list.find('.viewport').css('height',itemNo*25+'px');
				}
				
				function selectBoxEventHandler(e){
					//hide other open customSelectBox
				    $('.customSelectBox').not($(e.target).parents('.customSelectBox')).find('.customSelectBoxScroll').filter(':visible').hide();
					list.toggle(); // hide show list
					if(list.is(':visible')) {
						if((isKindle === false) && itemNo > 4){
							list.tinyscrollbar();
						}
						else {
							list.find('.viewport').css('width','100%');
							list.find('.scrollbar').hide();
							
						}
						parentAttr.css('z-index','2');
						parentAttr.siblings().css('z-index','1');
						productVariationDiv.css('z-index','2');
					}
					else {
						parentAttr.css('z-index','auto');
						parentAttr.siblings().css('z-index','auto');
						productVariationDiv.css('z-index','auto');
					}
				}
				
			textField.off("click").on("click", function(e){
				selectBoxEventHandler(e);
			});
			...
			item.off("click").on("click", function(){ // click on item
		 ...
		 //Hiding open selectbox
		$("body").on("click", function(e){
			if(($(e.target).parents('.customSelectBox').length === 0) && $('.customSelectBox').find('.customSelectBoxScroll').is(':visible')){
				$('.customSelectBox').find('.customSelectBoxScroll').hide();
				$('.customSelectBox').parents('.product-variations').css('z-index','auto');
				$('.customSelectBox').parents('.attribute').css('z-index','auto');
			}
		});
		return $(this);
	};
